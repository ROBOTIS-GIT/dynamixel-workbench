#include "dynamixel_workbench_controllers/dynamixel_workbench_multi_port_control.h"

using namespace dynamixel_workbench_multi_port_control;

DynamixelWorkbenchMultiPortControl::DynamixelWorkbenchMultiPortControl()
    :nh_priv_("~"),
     is_debug_(false),
     pan_motor_device_name_(""),
     pan_motor_baud_rate_(0.0),
     pan_motor_model_(""),
     pan_motor_id_(0),
     pan_motor_protocol_version_(0.0),
     tilt_motor_device_name_(""),
     tilt_motor_baud_rate_(0.0),
     tilt_motor_model_(""),
     tilt_motor_id_(0),
     tilt_motor_protocol_version_(0.0)
{
  ...
  // Init target name
  ROS_ASSERT(initDynamixelWorkbenchMultiPortControl());
  ...
  // init ROS Publish
  dxl_state_pub_ = nh_.advertise<dynamixel_workbench_msgs::MotorStateList>("/dynamixel_workbench_multi_port_control/motor_state",10);

  // init ROS Server
  multi_port_control_server = nh_.advertiseService("/dynamixel_workbench_tutorials/pan_tilt", &DynamixelWorkbenchMultiPortControl::controlPanTiltMotorCallback, this);
...
DynamixelWorkbenchMultiPortControl::~DynamixelWorkbenchMultiPortControl()
{
  ROS_ASSERT(shutdownDynamixelWorkbenchMultiPortControl());
}

bool DynamixelWorkbenchMultiPortControl::initDynamixelWorkbenchMultiPortControl(void)
{
  ROS_INFO("dynamixel_workbench_multi_port_control : Init OK!");
  return true;
}

bool DynamixelWorkbenchMultiPortControl::shutdownDynamixelWorkbenchMultiPortControl(void)
{
  writeTorque(false);
...
bool DynamixelWorkbenchMultiPortControl::initMotor(std::string motor_model, uint8_t motor_id, float protocol_version)
{
...
bool DynamixelWorkbenchMultiPortControl::writeDynamixelRegister(dynamixel::PortHandler *portHandler, dynamixel::PacketHandler *packetHandler, uint8_t id, uint16_t addr, uint8_t length, int64_t value)
{
  uint8_t dxl_error = 0;
...
bool DynamixelWorkbenchMultiPortControl::readDynamixelRegister(dynamixel::PortHandler *portHandler, dynamixel::PacketHandler *packetHandler, uint8_t id, uint16_t addr, uint8_t length, int64_t *value)
{
  uint8_t dxl_error = 0;
  ...
  if (length == 1)
  {
    dxl_comm_result_ = packetHandler->read1ByteTxRx(portHandler, id, addr, (uint8_t*)&value_8_bit, &dxl_error);
  }
  else if (length == 2)
  {
    dxl_comm_result_ = packetHandler->read2ByteTxRx(portHandler, id, addr, (uint16_t*)&value_16_bit, &dxl_error);
  }
  else if (length == 4)
  {
    dxl_comm_result_ = packetHandler->read4ByteTxRx(portHandler, id, addr, (uint32_t*)&value_32_bit, &dxl_error);
  }
...
bool DynamixelWorkbenchMultiPortControl::writeTorque(bool onoff)
{
  ...
  if (onoff == true)
  {
    writeDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, true);
    writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, true);
  }
  else
  {
    writeDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, false);
    writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, false);
  }
}

bool DynamixelWorkbenchMultiPortControl::writeProfile()
{
  if (!strncmp(pan_motor_model_.c_str(), "AX", 2) || !strncmp(pan_motor_model_.c_str(), "RX", 2) || !strncmp(pan_motor_model_.c_str(), "EX", 2))
  {
    dynamixel_[PAN_MOTOR]->dxl_item_ = dynamixel_[PAN_MOTOR]->ctrl_table_["moving_speed"];

    writeDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, VELOCITY);
  }
  else if (!strncmp(pan_motor_model_.c_str(), "MX", 2))
  {
    ...
    writeDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, ACCELERATION);
  }
  ...
  {
    dynamixel_[TILT_MOTOR]->dxl_item_ = dynamixel_[TILT_MOTOR]->ctrl_table_["moving_speed"];

    writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, VELOCITY);
  }
  else if (!strncmp(tilt_motor_model_.c_str(), "MX", 2))
  {
    ...
    dynamixel_[TILT_MOTOR]->dxl_item_ = dynamixel_[TILT_MOTOR]->ctrl_table_["goal_acceleration"];

    writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, ACCELERATION);
  }
  ...
  else
  {
    ...
    dynamixel_[TILT_MOTOR]->dxl_item_ = dynamixel_[TILT_MOTOR]->ctrl_table_["profile_acceleration"];

    writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, ACCELERATION);
 }
}

bool DynamixelWorkbenchMultiPortControl::writePosition(int64_t pan_pos, int64_t tilt_pos)
{
  ...
  writeDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, pan_pos);
  writeDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, tilt_pos);
}

bool DynamixelWorkbenchMultiPortControl::readMotorState(int8_t motor, std::string addr_name)
{
  int64_t read_value;
  ...
  if (motor == PAN_MOTOR)
  {
    ...
    readDynamixelRegister(pan_motor_portHandler_, pan_motor_packetHandler_, pan_motor_id_, dynamixel_[PAN_MOTOR]->dxl_item_->address, dynamixel_[PAN_MOTOR]->dxl_item_->data_length, &read_value);
  ...
  else
  {
    ...
    readDynamixelRegister(tilt_motor_portHandler_, tilt_motor_packetHandler_, tilt_motor_id_, dynamixel_[TILT_MOTOR]->dxl_item_->address, dynamixel_[TILT_MOTOR]->dxl_item_->data_length, &read_value);
...
bool DynamixelWorkbenchMultiPortControl::getPublishedMsg(void)
{
...
int64_t DynamixelWorkbenchMultiPortControl::convertRadian2Value(int8_t motor, double radian)
{
  int64_t value = 0;
  if (radian > 0)
  {
...
bool DynamixelWorkbenchMultiPortControl::dynamixelControlLoop(void)
{
  getPublishedMsg();
  ...
  dxl_response[PAN_MOTOR].motor_model = dynamixel_[PAN_MOTOR]->model_name_;
  dxl_response[PAN_MOTOR].id = dynamixel_[PAN_MOTOR]->id_;
  dxl_response[PAN_MOTOR].torque_enable = read_pan_motor_data_["torque_enable"];
  dxl_response[PAN_MOTOR].present_position = read_pan_motor_data_["present_position"];
  dxl_response[PAN_MOTOR].present_velocity = read_pan_motor_data_["present_velocity"];
  dxl_response[PAN_MOTOR].goal_position = read_pan_motor_data_["goal_position"];
  dxl_response[PAN_MOTOR].moving = read_pan_motor_data_["moving"];

  if (!strncmp(pan_motor_model_.c_str(), "AX", 2) || !strncmp(pan_motor_model_.c_str(), "RX", 2) || !strncmp(pan_motor_model_.c_str(), "MX", 2) || !strncmp(pan_motor_model_.c_str(), "EX", 2))
  {
    dxl_response[PAN_MOTOR].moving_speed = read_pan_motor_data_["moving_speed"];
  }
  else
  {
    dxl_response[PAN_MOTOR].goal_velocity = read_pan_motor_data_["goal_velocity"];
  }

  if (!strncmp(pan_motor_model_.c_str(), "MX", 2) || !strncmp(pan_motor_model_.c_str(), "PRO", 3))
  {
    dxl_response[PAN_MOTOR].goal_acceleration = read_pan_motor_data_["goal_acceleration"];
  }

  if (!strncmp(pan_motor_model_.c_str(), "XM", 2))
  {
    dxl_response[PAN_MOTOR].profile_velocity = read_pan_motor_data_["profile_velocity"];
    dxl_response[PAN_MOTOR].profile_acceleration = read_pan_motor_data_["profile_acceleration"];
  }

  if (!strncmp(pan_motor_model_.c_str(), "XM", 2) || !strncmp(pan_motor_model_.c_str(), "PRO", 3))
  {
    dxl_response[PAN_MOTOR].max_position_limit = read_pan_motor_data_["max_position_limit"];
    dxl_response[PAN_MOTOR].min_position_limit = read_pan_motor_data_["min_position_limit"];
  }
  else
  {
    dxl_response[PAN_MOTOR].cw_angle_limit = read_pan_motor_data_["cw_angle_limit"];
    dxl_response[PAN_MOTOR].ccw_angle_limit = read_pan_motor_data_["ccw_angle_limit"];
  }

  dxl_response_list.motor_states.push_back(dxl_response[PAN_MOTOR]);

  dxl_response[TILT_MOTOR].motor_model = dynamixel_[TILT_MOTOR]->model_name_;
  dxl_response[TILT_MOTOR].id = dynamixel_[TILT_MOTOR]->id_;
  dxl_response[TILT_MOTOR].torque_enable = read_tilt_motor_data_["torque_enable"];
  dxl_response[TILT_MOTOR].present_position = read_tilt_motor_data_["present_position"];
  dxl_response[TILT_MOTOR].present_velocity = read_tilt_motor_data_["present_velocity"];
  dxl_response[TILT_MOTOR].goal_position = read_tilt_motor_data_["goal_position"];
  dxl_response[TILT_MOTOR].moving = read_tilt_motor_data_["moving"];

  if (!strncmp(tilt_motor_model_.c_str(), "AX", 2) || !strncmp(tilt_motor_model_.c_str(), "RX", 2) || !strncmp(tilt_motor_model_.c_str(), "MX", 2) || !strncmp(tilt_motor_model_.c_str(), "EX", 2))
  {
    dxl_response[TILT_MOTOR].moving_speed = read_tilt_motor_data_["moving_speed"];
  }
  else
  {
    dxl_response[TILT_MOTOR].goal_velocity = read_tilt_motor_data_["goal_velocity"];
  }

  if (!strncmp(tilt_motor_model_.c_str(), "MX", 2) || !strncmp(tilt_motor_model_.c_str(), "PRO", 3))
  {
    dxl_response[TILT_MOTOR].goal_acceleration = read_tilt_motor_data_["goal_acceleration"];
  }

  if (!strncmp(tilt_motor_model_.c_str(), "XM", 2))
  {
    dxl_response[TILT_MOTOR].profile_velocity = read_tilt_motor_data_["profile_velocity"];
    dxl_response[TILT_MOTOR].profile_acceleration = read_tilt_motor_data_["profile_acceleration"];
  }

  if (!strncmp(tilt_motor_model_.c_str(), "XM", 2) || !strncmp(tilt_motor_model_.c_str(), "PRO", 3))
  {
    dxl_response[TILT_MOTOR].max_position_limit = read_tilt_motor_data_["max_position_limit"];
    dxl_response[TILT_MOTOR].min_position_limit = read_tilt_motor_data_["min_position_limit"];
  }
  else
  {
    dxl_response[TILT_MOTOR].cw_angle_limit = read_tilt_motor_data_["cw_angle_limit"];
    dxl_response[TILT_MOTOR].ccw_angle_limit = read_tilt_motor_data_["ccw_angle_limit"];
  }

  dxl_response_list.motor_states.push_back(dxl_response[TILT_MOTOR]);

  dxl_state_pub_.publish(dxl_response_list);
}

bool DynamixelWorkbenchMultiPortControl::controlPanTiltMotorCallback(dynamixel_workbench_msgs::SetPosition::Request &req,
...
{
  // Init ROS node
  ros::init(argc, argv, "dynamixel_workbench_multi_port_control");
  DynamixelWorkbenchMultiPortControl dxl_multi_port_ctrl;
  ros::Rate loop_rate(125);

  while (ros::ok())
  {
    dxl_multi_port_ctrl.dynamixelControlLoop();
    ros::spinOnce();
    loop_rate.sleep();
  }
